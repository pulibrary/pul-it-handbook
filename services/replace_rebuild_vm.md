...
- Go to the Princeton University Library [ansible tower](https://ansible-tower.princeton.edu/#/templates) and select _templates_ from the left hand side menu
- Search for a template called ["replace a VM (Originally in staging)"](https://ansible-tower.princeton.edu/#/templates/job_template/49/details)
- Select "launch"
- For `source control branch` we leave the value as `main`
- Enter the host name in the "VM to replace" field of the main branch without `.lib.princeton.edu` (ex. `orcid-staging2`)
...
- Next click the `Launch the Playbook` button

- To ensure that the playbook ran correctly ssh into the vm (ex. run `ssh pulsys@pdc-discovery-staging1.princeton.edu`)

## Update the ssh keys

- If we have new developers, opening a new Pull Request to that repo and re-running the `just build` command will add the new dev to the image.
