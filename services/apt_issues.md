...
1. run `playbooks/utils/fix_apt_issues.yml` on the affected machines, passing `-e recv_key=<key-hash>`
2. run `playbooks/utils/os_updates.yml` (in Tower the Template is called OS Updates Patch) on the affected machines to confirm the fix, update the cache, and upgrade all packages
